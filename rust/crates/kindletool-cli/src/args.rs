...
use kindletool::devices::DeviceCatalog;
use kindletool::model::{Board, BundleMagic, Certificate, Platform};
...
fn create_catalog_help() -> String {
    ...
    for (platform, cli_name, display_name) in Platform::known() {
        let _ = writeln!(output, "  {cli_name:<18} {display_name} ({})", platform.0);
    }

    output.push_str("\nBoards:\n");
    for (board, cli_name, display_name) in Board::known() {
        let _ = writeln!(output, "  {cli_name:<18} {display_name} ({})", board.0);
    }
...
pub(crate) struct ConvertArgs {
    /// Write converted payloads to stdout and keep source packages.
    #[arg(short = 'c', long = "stdout")]
    pub(crate) stdout: bool,
    /// Inspect only; do not write converted payloads.
    #[arg(short = 'i', long = "info")]
    pub(crate) inspect: bool,
    /// Keep source packages after successful conversion.
    #[arg(short = 'k', long = "keep")]
    pub(crate) keep: bool,
    /// Extract only the outer SP01 signature.
    #[arg(short = 's', long = "sig")]
    pub(crate) signature: bool,
    /// Treat the package payload as unmangled.
    #[arg(short = 'u', long = "unsigned")]
    pub(crate) fake_sign: bool,
    /// Remove one SP01 envelope without decoding the inner package.
    #[arg(short = 'w', long = "unwrap")]
    pub(crate) unwrap: bool,
    /// Input packages; `-` reads one package from stdin and writes to stdout.
    #[arg(required = true)]
...
pub(crate) struct CreateArgs {
    ...
    /// Kindle signing certificate number.
...
pub(crate) struct InfoArgs {
    /// Sixteen-character Kindle serial number.
...
mod tests {
    ...
    fn legacy_create_long_options_are_preserved() {
        ...
        assert!(args.packaging_metadata);
        assert!(args.keep_archive);
        assert!(args.official_ota);
        assert!(args.legacy_paths);
    }
